...
// ── MANGU ACTIVITY MODULE — Parche 3.1 ──────────────────────────────────────
// Heatmap anual + tiempo por serie + logros en el perfil propio
// Carga después de community.js. No modifica ningún archivo existente.
...
// Llama a logManguActivity(serie, delta) cada vez que se marcan caps/eps.
// Usa activityLog: [{date:"YYYY-MM-DD", count:N}] en cada serie.
// Si tracker.js ya llama a save() luego de marcar, solo agrega logManguActivity antes.
...
function renderActivitySection(){
  ...
  const cb  = v=>{
    if(!v) return null;
    const r=v/maxV;
    return r<.2?PAL[0]:r<.45?PAL[1]:r<.75?PAL[2]:PAL[3];
  };

  // ── KPIs ──────────────────────────────────────────────────────────────────
  const pipHtml = Array.from({length:Math.min(streak,14)}, (_,i)=>
    `<div style="height:4px;width:18px;border-radius:2px;background:${i<streak?"#6366f1":"rgba(255,255,255,.08)"}"></div>`
  ).join("");
...
<div style="margin-top:14px">
  ...
  <!-- tabs -->
  <div style="display:flex;gap:2px;margin-bottom:14px;background:rgba(255,255,255,.04);border-radius:12px;padding:3px" id="mg-act-tabbar">
  ...
  spacer.style.cssText = "flex-shrink:0;width:18px;margin-right:3px";
  ...
  Object.entries(actMap).forEach(([k,v])=>{
    if(k.startsWith(String(YEAR))){ monthTotals[parseInt(k.slice(5,7))-1]+=v; }
  });
  ...
  monthTotals.forEach((v,i)=>{
    ...
    bar.style.cssText = `width:100%;border-radius:4px 4px 0 0;min-height:3px;transition:opacity .15s;cursor:default;height:${v>0?Math.max(Math.round((v/maxM)*100),7):4}%;background:${v>0?BCOLS[i]:"rgba(255,255,255,.07)"}`;
    bar.addEventListener("mouseenter",()=>bar.style.opacity=".65");
    bar.addEventListener("mouseleave",()=>bar.style.opacity="1");
    col.appendChild(bar);
    mbRow.appendChild(col);
    ...
    lbl.textContent = shortM[i];
    mbLbls.appendChild(lbl);
  });
  ...
  activeAll.forEach((s,idx)=>{
    ...
    const bg   = color.startsWith("#") ? hexAlpha(color,.12) : "rgba(255,255,255,.06)";
    const init = (s.title||"?").slice(0,2).toUpperCase();
    const card = document.createElement("div");
    card.style.cssText = `border-radius:14px;border:1px solid rgba(255,255,255,.07);padding:12px 14px;display:flex;align-items:center;gap:12px;transition:border-color .18s;cursor:default;background:${hexAlpha(color,.05)}`;
    card.addEventListener("mouseenter",()=>card.style.borderColor="rgba(255,255,255,.16)");
    card.addEventListener("mouseleave",()=>card.style.borderColor="rgba(255,255,255,.07)");
    card.innerHTML=`
      <div style="width:40px;height:54px;border-radius:9px;background:${bg};color:${color};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0;font-family:'Space Mono',monospace;border:1px solid ${hexAlpha(color,.18)}">${init}</div>
  ...
  // ── LOGROS ────────────────────────────────────────────────────────────────
  const lg = document.getElementById("mg-logros-list");
  const logros = buildLogros(streak, totalCaps, mangaCaps, animeCaps, hours,
    (data.manga||[]).length, (data.anime||[]).length,
    [...(data.manga||[]),...(data.anime||[])].filter(s=>s.status==="completed").length);
  logros.forEach(l=>{
    const row = document.createElement("div");
    row.style.cssText="display:flex;align-items:center;gap:11px;padding:10px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.018);margin-bottom:7px;transition:background .15s;cursor:default";
    row.addEventListener("mouseenter",()=>row.style.background="rgba(255,255,255,.04)");
    row.addEventListener("mouseleave",()=>row.style.background="rgba(255,255,255,.018)");
    row.innerHTML=`
      ...
      ${l.unlocked
        ...
        :`<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.2)" stroke-width="2" stroke-linecap="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`}`;
    lg.appendChild(row);
  });
}

function buildLogros(streak, totalCaps, mangaCaps, animeCaps, hours, mangaCount, animeCount, completedCount){
  const SVG = {
  ...
  return [
    {label:"Primera racha",         sub:`${streak} día${streak>1?"s":""} consecutivos`,     unlocked:streak>=1,   bg:"rgba(249,115,22,.15)", c:"#fb923c", svg:SVG.flame},
    {label:"Racha de fuego",         sub:"7 días seguidos",                                  unlocked:streak>=7,   bg:"rgba(239,68,68,.15)",  c:"#f87171", svg:SVG.flame},
    {label:"Racha legendaria",       sub:"30 días seguidos",                                 unlocked:streak>=30,  bg:"rgba(245,158,11,.15)", c:"#fbbf24", svg:SVG.flame},
    {label:"Lector constante",       sub:`${totalCaps} caps/eps marcados`,                   unlocked:totalCaps>=100,  bg:"rgba(99,102,241,.15)", c:"#818cf8", svg:SVG.book},
    {label:"Devorador de manga",     sub:"500+ caps de manga",                               unlocked:mangaCaps>=500,  bg:"rgba(129,140,248,.15)",c:"#a5b4fc", svg:SVG.book},
    {label:"Maratonista",            sub:"100+ eps de anime",                                unlocked:animeCaps>=100,  bg:"rgba(6,182,212,.15)", c:"#22d3ee",  svg:SVG.tv},
    {label:"100 horas",              sub:`${hours}h de lectura acumuladas`,                  unlocked:hours>=100,      bg:"rgba(139,92,246,.15)",c:"#c4b5fd", svg:SVG.clock},
    {label:"Coleccionista",          sub:`${mangaCount+animeCount} series en lista`,         unlocked:(mangaCount+animeCount)>=10, bg:"rgba(34,197,94,.14)", c:"#4ade80", svg:SVG.stack},
    {label:"Completionista",         sub:`${completedCount} series completadas`,             unlocked:completedCount>=5, bg:"rgba(245,158,11,.14)",c:"#fcd34d", svg:SVG.check},
    {label:"Otaku certificado",      sub:"Manga + anime activos al mismo tiempo",            unlocked:mangaCount>0&&animeCount>0, bg:"rgba(236,72,153,.14)",c:"#f472b6", svg:SVG.heart},
    {label:"Puntuador exigente",     sub:"10+ series con score asignado",                    unlocked:[...(data.manga||[]),...(data.anime||[])].filter(s=>s.score>0).length>=10, bg:"rgba(251,191,36,.14)",c:"#fde68a", svg:SVG.star},
  ];
}
...
// Se añade el <div id="mg-activity-root"> después de la my-profile-card.
// La función observa el DOM hasta que aparece el perfil propio.
...
// Make hexAlpha accessible in render scope
window._mgHexAlpha = hexAlpha;

function mountRoot(){
  // Si ya existe el root, solo re-renderizar
  if(document.getElementById("mg-activity-root")){
    renderActivitySection();
    return;
  }
  // Buscar la tarjeta de mi perfil
...
// Observer: espera a que renderCommunity() haya puesto .my-profile-card en el DOM
const _mgObs = new MutationObserver(()=>{
  if(document.querySelector(".my-profile-card")){
    mountRoot();
  }
});
_mgObs.observe(document.body, {childList:true, subtree:true});

// Si ya estaba montado al cargar (navegación entre tabs)
if(document.querySelector(".my-profile-card")){
  setTimeout(()=>{ injectCSS(); mountRoot(); }, 50);
}
