// ── MANGU ACTIVITY MODULE — Parche 3.3 ──────────────────────────────────────
// Heatmap + tiempo por serie + logros en el perfil propio
// Estrategia: polling liviano sobre friends-panel-container para detectar
// cuando my-profile-card aparece, sin depender de monkey-patch ni MutationObserver.
// ────────────────────────────────────────────────────────────────────────────
...
// ── 2. PATCH migrate() para persistir activityLog ────────────────────────────
...
function currentStreak(m){
  ...
  while(true){ const k=d.toISOString().slice(0,10); if(!m[k]) break; streak++; d.setDate(d.getDate()-1); }
  return streak;
}

function seriesAge(s){
  const ref=s.startDate?new Date(s.startDate).getTime():(s.createdAt||parseInt(s.id)||Date.now());
  const days=Math.max(0,Math.floor((Date.now()-ref)/86400000));
  return { days, weeks:Math.floor(days/7), months:(days/30.44).toFixed(1),
...
<div id="mg-act-hm">
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px">
    <div class="mg-kpi" style="background:rgba(99,102,241,.1);border-color:rgba(99,102,241,.22)">
      <div class="mg-kl" style="color:rgba(165,180,252,.55)">Racha</div>
      <div class="mg-kv" style="color:#a5b4fc">${streak}<span class="mg-ku"> d</span></div>
      <div style="display:flex;gap:3px;margin-top:6px;flex-wrap:wrap">${pipHtml}</div>
    </div>
    <div class="mg-kpi" style="background:rgba(6,182,212,.08);border-color:rgba(6,182,212,.2)">
      <div class="mg-kl" style="color:rgba(103,232,249,.55)">Total</div>
      <div class="mg-kv" style="color:#67e8f9">${hours}<span class="mg-ku"> h</span></div>
      <div class="mg-ks">${totalCaps} caps</div>
    </div>
    <div class="mg-kpi" style="background:rgba(139,92,246,.09);border-color:rgba(139,92,246,.2)">
      <div class="mg-kl" style="color:rgba(196,181,253,.55)">Semana</div>
      <div class="mg-kv" style="color:#c4b5fd">${wk}<span class="mg-ku"></span></div>
      <div class="mg-ks">caps/eps</div>
    </div>
    <div class="mg-kpi" style="background:rgba(34,197,94,.07);border-color:rgba(34,197,94,.18)">
      <div class="mg-kl" style="color:rgba(134,239,172,.55)">Mes</div>
      <div class="mg-kv" style="color:#86efac">${mo}<span class="mg-ku"></span></div>
      <div class="mg-ks">${mn[today.getMonth()]}</div>
    </div>
  </div>
...
<div id="mg-act-logros" style="display:none"><div id="mg-ll"></div></div>`;
  ...
  // HEATMAP
  const hmC=document.getElementById("mg-hm-c");
  ...
  const spc=document.createElement("div"); spc.style.cssText="flex-shrink:0;width:18px;margin-right:3px";
  mrow.appendChild(spc);
  let lastM=-1;
  for(let w=0;w<WEEKS;w++){
    const di=w*7-startDow, d=new Date(YEAR,0,1+di), m=d.getMonth();
    const sp=document.createElement("div"); sp.style.cssText="flex:1;min-width:0;overflow:hidden;text-align:left";
    sp.textContent=(m!==lastM&&di>=0&&di<totalDays)?mn[m].slice(0,1):""; lastM=m; mrow.appendChild(sp);
  }
  ...
  for(let w=0;w<WEEKS;w++){
    ...
    for(let dow=0;dow<7;dow++){
      const di=w*7+dow-startDow, date=new Date(YEAR,0,1+di);
      const cell=document.createElement("div");
      const valid=date.getFullYear()===YEAR&&di>=0&&di<totalDays;
      const v=valid?(actMap[date.toISOString().slice(0,10)]||0):0;
      cell.style.cssText=`width:100%;height:11px;border-radius:2px;background:${valid?(cb(v)||"rgba(255,255,255,.06)"):"transparent"};transition:transform .1s;cursor:default`;
      if(valid){
        ...
        cell.addEventListener("mouseleave",()=>{ cell.style.transform="scale(1)"; tip.textContent="› pasa el cursor sobre un día"; tip.style.color="rgba(255,255,255,.25)"; });
      }
  ...
  Object.entries(actMap).forEach(([k,v])=>{ if(k.startsWith(String(YEAR))) mT[parseInt(k.slice(5,7))-1]+=v; });
  const maxM=Math.max(...mT,1);
  const sM=["E","F","M","A","M","J","J","A","S","O","N","D"];
  const BC=["#6366f1","#818cf8","#4f46e5","#6366f1","#7c3aed","#818cf8","#6366f1","#4f46e5","#818cf8","#6366f1","#7c3aed","#4f46e5"];
  const mbR=document.getElementById("mg-mb-r"), mbL=document.getElementById("mg-mb-l");
  mT.forEach((v,i)=>{
    const col=document.createElement("div"); col.style.cssText="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:3px;height:100%";
    const bar=document.createElement("div");
    bar.style.cssText=`width:100%;border-radius:4px 4px 0 0;min-height:3px;cursor:default;transition:opacity .15s;height:${v>0?Math.max(Math.round((v/maxM)*100),7):4}%;background:${v>0?BC[i]:"rgba(255,255,255,.07)"}`;
    bar.addEventListener("mouseenter",()=>bar.style.opacity=".65");
    bar.addEventListener("mouseleave",()=>bar.style.opacity="1");
    col.appendChild(bar); mbR.appendChild(col);
    const lbl=document.createElement("div"); lbl.style.cssText="flex:1;font-size:8px;color:rgba(255,255,255,.2);font-family:'Space Mono',monospace;text-align:center";
    lbl.textContent=sM[i]; mbL.appendChild(lbl);
  });

  // TIEMPO
  const tg=document.getElementById("mg-tg");
  const SC=["#f97316","#8b5cf6","#e24b4a","#06b6d4","#22c55e","#ef4444","#fbbf24","#ec4899","#14b8a6","#6366f1"];
  const active=[...(data.manga||[]).filter(s=>s.status==="reading").map(s=>({...s,_t:"Manga"})),
                ...(data.anime||[]).filter(s=>s.status==="reading").map(s=>({...s,_t:"Anime"}))]
    .sort((a,b)=>(b.lastUpdated||0)-(a.lastUpdated||0)).slice(0,6);
  if(active.length===0){
    tg.innerHTML=`<div style="grid-column:1/-1;font-size:12px;color:rgba(255,255,255,.28);text-align:center;padding:20px">Sin series activas</div>`;
  } else {
    active.forEach((s,i)=>{
      const age=seriesAge(s), pct=s.total>0?Math.round((s.completed.length/s.total)*100):0;
      const c=SC[i%SC.length], init=(s.title||"?").slice(0,2).toUpperCase();
      const card=document.createElement("div"); card.className="mg-tc"; card.style.cssText=`background:${hexA(c,.05)}`;
      card.innerHTML=`
        <div style="width:40px;height:54px;border-radius:9px;background:${hexA(c,.13)};color:${c};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0;font-family:'Space Mono',monospace;border:1px solid ${hexA(c,.2)}">${init}</div>
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:${c};opacity:.7;margin-bottom:6px">${s._t}</div>
          <div style="height:3px;background:rgba(255,255,255,.07);border-radius:2px;overflow:hidden;margin-bottom:7px"><div style="width:${pct}%;height:100%;border-radius:2px;background:${c}"></div></div>
          <div style="font-size:20px;font-weight:800;font-family:'Space Mono',monospace;line-height:1;letter-spacing:-.03em;color:${c}">${age.label}</div>
  ...
  const scoredCount=allSeries.filter(s=>s.score>0).length;
  const completedCount=allSeries.filter(s=>s.status==="completed").length;
  [
    {label:"Primera racha",      sub:`${streak} día${streak!==1?"s":""} consecutivos`,  unlocked:streak>=1,    bg:hexA("#f97316",.15),c:"#fb923c",svg:SV.flame},
    {label:"Racha de fuego",     sub:"7 días seguidos",                                  unlocked:streak>=7,    bg:hexA("#ef4444",.15),c:"#f87171",svg:SV.flame},
    {label:"Racha legendaria",   sub:"30 días seguidos",                                 unlocked:streak>=30,   bg:hexA("#f59e0b",.15),c:"#fbbf24",svg:SV.flame},
    {label:"Lector constante",   sub:`${totalCaps} caps/eps marcados`,                   unlocked:totalCaps>=100,bg:hexA("#6366f1",.15),c:"#818cf8",svg:SV.book},
    {label:"Devorador de manga", sub:"500+ caps de manga",                               unlocked:mangaCaps>=500,bg:hexA("#8b5cf6",.15),c:"#a5b4fc",svg:SV.book},
    {label:"Maratonista",        sub:"100+ eps de anime",                                unlocked:animeCaps>=100,bg:hexA("#06b6d4",.15),c:"#22d3ee",svg:SV.tv},
    {label:"100 horas",          sub:`${hours}h acumuladas`,                             unlocked:hours>=100,   bg:hexA("#8b5cf6",.15),c:"#c4b5fd",svg:SV.clock},
    {label:"Coleccionista",      sub:`${allSeries.length} series en lista`,              unlocked:allSeries.length>=10,bg:hexA("#22c55e",.14),c:"#4ade80",svg:SV.stack},
    {label:"Completionista",     sub:`${completedCount} series completadas`,             unlocked:completedCount>=5,bg:hexA("#f59e0b",.14),c:"#fcd34d",svg:SV.check},
    {label:"Otaku certificado",  sub:"Manga + anime activos",                            unlocked:(data.manga||[]).length>0&&(data.anime||[]).length>0,bg:hexA("#ec4899",.14),c:"#f472b6",svg:SV.heart},
    {label:"Puntuador exigente", sub:"10+ series con score",                             unlocked:scoredCount>=10,bg:hexA("#f59e0b",.14),c:"#fde68a",svg:SV.star},
  ].forEach(l=>{
    const row=document.createElement("div"); row.className="mg-lg";
    row.innerHTML=`
...
window.mgActSwitch=function(id,btn){
  ["hm","tiempo","logros"].forEach(t=>{ const e=document.getElementById("mg-act-"+t); if(e) e.style.display=t===id?"block":"none"; });
  document.querySelectorAll(".mg-at").forEach(b=>b.classList.remove("on"));
  if(btn) btn.classList.add("on");
};

// ── 7. MONTE GARANTIZADO ──────────────────────────────────────────────────────
// Polling liviano: cada 300ms comprueba si my-profile-card está en el DOM.
// Se detiene solo cuando lo encuentra e inyecta el root.
// También re-renderiza si el root ya existe (por si cambia el tab y vuelve).

let _pollTimer=null;

function tryMount(){
  const card=document.querySelector(".my-profile-card");
  if(!card) return false; // todavía no está

  // ¿Ya existe el root justo después de esta card?
...
function startPolling(){
  ...
  _pollTimer=setInterval(()=>{
    if(tryMount()){
      // Encontrado — reducir frecuencia a 2s para re-renders al volver al tab
      clearInterval(_pollTimer);
      _pollTimer=setInterval(()=>{
        const card=document.querySelector(".my-profile-card");
        const root=document.getElementById("mg-activity-root");
        if(card&&!root){
          // card existe pero root fue eliminado (re-render del panel) — remontar
          const newRoot=document.createElement("div"); newRoot.id="mg-activity-root";
          card.parentNode.insertBefore(newRoot, card.nextSibling);
          injectCSS(); renderActivitySection();
        } else if(card&&root&&!root.parentNode){
          injectCSS(); renderActivitySection();
        }
      }, 2000);
    }
